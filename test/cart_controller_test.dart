...
void main() {
  // GetX resolves controllers through the container, so it has to be seeded
  // before `CartController.to` can be used.
  ...
  test('charges shipping below the free-shipping threshold', () {
    ...
    expect(cart.subtotal, lessThan(CartController.freeShippingThreshold));
    expect(cart.shippingFee, CartController.shippingRate);
  ...
  test('gives free shipping at or above the threshold', () {
    ...
    expect(cart.subtotal, CartController.freeShippingThreshold);
